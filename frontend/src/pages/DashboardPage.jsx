...
function Card({ children, className = "", delay = 0, noHover = false, onClick }) {
...
export default function DashboardPage() {
  ...
  const convRate  = total > 0 ? (((byStatus.interview ?? 0) / total) * 100).toFixed(1) : "0.0";
  const offerRate = total > 0 ? (((byStatus.offer ?? 0) / total) * 100).toFixed(1) : "0.0";
    ...
    >
      <div className="mx-auto max-w-6xl">
        ...
        <motion.div className="mb-8" variants={fadeUp(0)}>
          <p className="text-[0.6rem] font-semibold uppercase tracking-[0.12em] text-[#A8A29E] mb-1">{today}</p>
          ...
          {STAT_STRIP.map(({ key, label, dot }, idx) => (
            ...
            >
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-[0.58rem] font-bold uppercase tracking-widest text-[#A8A29E]">{label}</span>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6">
          ...
          <div className="flex flex-col gap-6">
            ...
            <Card className="p-7">
              ...
              <div className="space-y-4">
                {FUNNEL.map(({ key, label, fill, track }) => {
                  ...
                  const ofTotal = total > 0 ? ((val / total) * 100).toFixed(0) : 0;
                  return (
                    <div key={key} className="flex items-center gap-4">
                      <span className="text-sm font-medium text-[#57534E] w-16 flex-shrink-0">{label}</span>
                      ...
                      <span className="text-sm font-semibold text-stone-600 w-5 text-right flex-shrink-0">{val}</span>
                      <span className="text-[0.65rem] text-[#A8A29E] w-7 text-right flex-shrink-0">{ofTotal}%</span>
                    </div>
                  );
                })}
              </div>
            ...
            <Card className="p-7">
              <div className="flex justify-between items-center mb-5">
                ...
                <button className="text-xs font-medium text-[#A8A29E] hover:text-stone-700 transition-colors flex items-center gap-1 -mt-4">
                  View all <ArrowRight size={11} />
                </button>
              </div>
              {recentJobs.length === 0 ? (
                <p className="text-sm text-stone-300 text-center py-6">No applications yet</p>
              ) : (
                    ...
                    <div key={job.id} className="py-3.5 flex justify-between items-center gap-4">
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium text-stone-900 truncate flex items-center gap-1.5">
                          {job.starred && <Star size={11} fill="currentColor" className="text-amber-400 flex-shrink-0" />}
                          {job.title}
                        </p>
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0">
                        <span className={`px-2.5 py-0.5 rounded-full border text-[0.6rem] font-bold uppercase tracking-wider ${PILL_STYLES[job.status]}`}>
                          {job.status}
                        </span>
          ...
          <div className="flex flex-col gap-5">
            ...
            {/* Rate cards */}
            {[
              { label: "Apply → Interview", value: convRate,  sub: "vs. 15% industry avg", color: "#1C1917" },
              { label: "Offer Rate",         value: offerRate, sub: `${byStatus.offer || 0} offers · ${total} apps`, color: "#2D6A4F" },
            ].map(({ label, value, sub, color }) => (
              <Card key={label} className="p-6">
                <p className="text-[0.6rem] font-bold uppercase tracking-widest text-[#A8A29E] mb-2">{label}</p>
                ...
                <div className="h-1 bg-[#EFEDE9] rounded-full mt-4 overflow-hidden">
                  <motion.div
                    ...
                    animate={reduce ? {} : { width: `${Math.min(parseFloat(value), 100)}%` }}
            ...
            {starredJobs.length > 0 && (
              <Card className="p-6">
                <div className="flex items-center gap-1.5 mb-4">
                  ...
                  <p className="text-[0.6rem] font-bold uppercase tracking-widest text-[#A8A29E]">Starred</p>
                </div>
                <div className="space-y-3">
                  {starredJobs.map((job) => (
                    <div key={job.id} className="flex justify-between items-center gap-2">
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-stone-800 truncate">{job.title}</p>
                        <p className="text-[0.68rem] text-[#A8A29E]">{job.company}</p>
                      </div>
                      <span className={`flex-shrink-0 px-2 py-0.5 rounded-full border text-[0.58rem] font-bold uppercase tracking-wider ${PILL_STYLES[job.status]}`}>
                        {job.status}
                      </span>
                    </div>
                  ))}
            ...
            <Card className="p-6 bg-[#EFEDE9] border-[#E8E4DE]" noHover>
              <span className="text-[0.58rem] font-bold uppercase tracking-widest text-[#57534E]">✨ Insight</span>
